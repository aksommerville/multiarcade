...
#define MA_X11_KEY_REPEAT_INTERVAL 10

#define MA_X11_SCALE_LIMIT 16
...
static int ma_x11_init(struct ma_x11 *x11) {
...
struct ma_x11 *ma_x11_new(
  ...
  struct ma_x11 *x11=calloc(1,sizeof(struct ma_x11));
  if (!x11) return 0;
...
static int ma_x11_recalculate_output_bounds(struct ma_x11 *x11) {

  /* First decide the scale factor:
   *  - At least 1.
   *  - At most MA_X11_SCALE_LIMIT.
  ...
  else if (x11->scale>MA_X11_SCALE_LIMIT) x11->scale=MA_X11_SCALE_LIMIT;
...
int ma_x11_swap(struct ma_x11 *x11,const void *fb) {
  if (x11->dstdirty) {
    if (ma_x11_recalculate_output_bounds(x11)<0) return -1;
    x11->dstdirty=0;
...
static void ma_x11_enter_fullscreen(struct ma_x11 *x11) {
  XEvent evt={
    .xclient={
      .type=ClientMessage,
...
static void ma_x11_exit_fullscreen(struct ma_x11 *x11) {
  XEvent evt={
    .xclient={
      .type=ClientMessage,
      .message_type=x11->atom__NET_WM_STATE,
...
int ma_x11_set_fullscreen(struct ma_x11 *x11,int state) {
  if (state>0) {
    ...
    ma_x11_enter_fullscreen(x11);
  } else if (state==0) {
    if (!x11->fullscreen) return 0;
    ma_x11_exit_fullscreen(x11);
  }
...
static int ma_x11_receive_event(struct ma_x11 *x11,XEvent *evt) {
  if (!evt) return -1;
  switch (evt->type) {
    ...
    case KeyRelease: {
        ...
        uint16_t btnid=0;
        switch (keysym) {
          case XK_Down: btnid=MA_BUTTON_DOWN; break;
          case XK_Up: btnid=MA_BUTTON_UP; break;
          case XK_Left: btnid=MA_BUTTON_LEFT; break;
          case XK_Right: btnid=MA_BUTTON_RIGHT; break;
          case XK_z: btnid=MA_BUTTON_A; break;
          case XK_x: btnid=MA_BUTTON_B; break;
          case XK_Escape: if (x11->cb_close) return x11->cb_close(x11); return 0;
          case XK_F11: if (evt->type==KeyPress) return ma_x11_set_fullscreen(x11,x11->fullscreen?0:1); return 0;
        }
...
int ma_x11_update(struct ma_x11 *x11) {
  ...
  while (evtc-->0) {
    ...
    if ((evtc>0)&&(evt.type==KeyRelease)) {
      ...
      evtc--;
      if ((next.type==KeyPress)&&(evt.xkey.keycode==next.xkey.keycode)&&(evt.xkey.time>=next.xkey.time-MA_X11_KEY_REPEAT_INTERVAL)) {
        evt.type=KeyRepeat;
        if (ma_x11_receive_event(x11,&evt)<0) return -1;
      } else {
        if (ma_x11_receive_event(x11,&evt)<0) return -1;
        if (ma_x11_receive_event(x11,&next)<0) return -1;
      }
    } else {
      if (ma_x11_receive_event(x11,&evt)<0) return -1;
    }

...
struct ma_init_params ma_tiny_init_params={0};
...
static void ma_tiny_audio_init() {
  analogWrite(A0, 0);
  ...
  TC5->COUNT16.CC[0].reg = (SystemCoreClock / ma_tiny_init_params.audio_rate - 1);
  while(tcIsSyncing());
...
uint8_t ma_init(struct ma_init_params *params) {
  ...
  if (params) {
    params->videow=96;
    params->videoh=64;
    memcpy(&ma_tiny_init_params,params,sizeof(struct ma_init_params));
  } else {
    ma_tiny_init_params.videow=96;
    ma_tiny_init_params.videoh=64;
    ma_tiny_init_params.rate=0;
    ma_tiny_init_params.audio_rate=0;
  }
  ...
  if (ma_tiny_init_params.audio_rate) {
    ma_tiny_audio_init();
  }
...
uint16_t ma_update() {
  uint8_t state=0;
  if (analogRead(42)<0x08) state|=MA_BUTTON_UP;
  if (analogRead(19)<0x08) state|=MA_BUTTON_DOWN;
  if (analogRead(25)<0x08) state|=MA_BUTTON_LEFT;
  if (analogRead(15)<0x08) state|=MA_BUTTON_RIGHT;
  if (!digitalRead(44)) state|=MA_BUTTON_A;
  if (!digitalRead(45)) state|=MA_BUTTON_B;
  return state;
}
...
void ma_send_framebuffer(const void *fb) {
  ...
  for (int j=96*64*(MA_PIXELSIZE>>3);j-->0;FB++) {
